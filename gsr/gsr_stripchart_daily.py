...
class blk(gr.basic_block):  # other base classes are basic_block, decim_block, interp_block
    """A strip chart takes in a stream of floats and produces a 1 hour long
       ...
       Params:
          ...
          journee - how many journee long is the stripchart vector
    """

    def __init__(self, decim=100,seconds=3400,journee=86400):  # only default arguments here
        """arguments to this function show up as parameters in GRC"""
        gr.basic_block.__init__(
            self,
            name='BAA:Strip Chart Daily',   # will show up in GRC
            in_sig=[np.float32],
            out_sig=[(np.float32,journee)]
        )
